...
export const Sidebar: React.FC<SidebarProps> = React.memo(({ view, setView, mobileMenuOpen, session, handleLogout, setShowAuth, aiConfig }) => {
    return (
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-primary text-slate-300 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col">
          ...
          <nav className="flex-1 px-4 py-6 space-y-2">
            ...
            <SidebarItem view="subscription" currentView={view} setView={setView} icon={CreditCard} label="Abonnement" />
