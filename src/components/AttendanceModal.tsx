...
import { X, Check, UserX, Clock, Users, Calendar } from 'lucide-react';
import { Player, Attendance, Session } from '../types';

interface AttendanceModalProps {
  players: Player[];
  attendance: Attendance[];
  onToggle: (playerId: string, status: 'present' | 'absent' | 'late', sessionId: number) => void;
  onClose: () => void;
  groupName?: string;
  sessions?: Session[];
...
export const AttendanceModal: React.FC<AttendanceModalProps> = ({ 
  ...
  const [selectedSessionId, setSelectedSessionId] = useState<number | undefined>(initialSessionId || sessions[0]?.id);
  ...
  const currentAttendance = attendance.filter(a => a.session_id === selectedSessionId);
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl max-w-lg w-full overflow-hidden border border-slate-100 dark:border-slate-800">
          ...
          {sessions.length > 0 && (
            <div className="space-y-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2 flex items-center gap-1">
                <Calendar size={10}/> Choisir la séance
              </label>
              <select 
                value={selectedSessionId} 
                onChange={(e) => setSelectedSessionId(Number(e.target.value))}
                className="w-full p-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl text-xs font-bold dark:text-white outline-none focus:ring-2 focus:ring-accent/20"
              >
                {sessions.map(s => (
        ...
        <div className="p-6 max-h-[50vh] overflow-y-auto custom-scrollbar space-y-3">
          {!selectedSessionId ? (
            <p className="text-center py-8 text-slate-400 font-bold uppercase text-xs tracking-widest">Veuillez sélectionner ou créer une séance</p>
          ) : players.length === 0 ? (
            <p className="text-center py-8 text-slate-400 font-bold uppercase text-xs tracking-widest">Aucun joueur dans ce groupe</p>
          ) : players.map(player => {
              const record = currentAttendance.find(a => a.player_id === player.id);
              ...
              return (
                <div key={player.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 group">
                  ...
                  <div className="flex gap-1 bg-white dark:bg-slate-900 p-1 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800">
                    <button onClick={() => onToggle(player.id, 'present', selectedSessionId)} className={`p-2 rounded-lg transition-all ${status === 'present' ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-300 hover:text-emerald-500'}`} title="Présent"><Check size={18} /></button>
                    <button onClick={() => onToggle(player.id, 'late', selectedSessionId)} className={`p-2 rounded-lg transition-all ${status === 'late' ? 'bg-amber-500 text-white shadow-md' : 'text-slate-300 hover:text-amber-500'}`} title="En retard"><Clock size={18} /></button>
                    <button onClick={() => onToggle(player.id, 'absent', selectedSessionId)} className={`p-2 rounded-lg transition-all ${status === 'absent' ? 'bg-rose-500 text-white shadow-md' : 'text-slate-300 hover:text-rose-500'}`} title="Absent"><UserX size={18} /></button>
                  </div>
                </div>
              );
            })}
